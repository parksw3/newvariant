...
We further }\DIFdelend \DIFaddbegin \DIFadd{Here, we challenge a commonly-made assumption in estimating the transmission advantage of SARS-CoV-2 variants and }\DIFaddend argue that holding relative strength \DIFaddbegin \DIFadd{(rather than relative speed) }\DIFaddend constant is generally more appropriate\DIFdelbegin \DIFdel{for estimating the transmission advantage of new SARS-CoV-2 variants by comparing }\DIFdelend \DIFaddbegin \DIFadd{.
In doing so, we use the generation-interval-based framework to compare }\DIFaddend how relative strength and speed change over time under two idealized interventions\DIFdelbegin \DIFdel{of constant strength and speed.
...
Using the generation-interval-based framework further allows us to account for the possibility that new SARS-CoV-2 variants, such as the Alpha and Delta variants, may have different generation intervals from prior dominant lineages. 
Differences }\DIFaddend in the generation-interval \DIFdelbegin \DIFdel{distribution can alter }\DIFdelend \DIFaddbegin \DIFadd{distributions can affect }\DIFaddend the effectiveness of these interventions \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{as well as the realized transmission advantages of new variants.
Neglecting these differences gives biased estimates of relative strength, which can vary systematically with changes in the incidence of infections caused by the variant.
}\DIFaddend 
...
On the other hand, a constant-speed intervention reduces transmission after infection by a constant rate $\phi$: $K(\tau) \exp(-\phi \tau)$---in this case, the intervention speed $\phi$ must be greater than the epidemic speed $r$ to prevent onward transmission \mbox{%DIFAUXCMD
\citep{doi:10.1098/rspb.2020.1556}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFaddend 
...
For example, under a constant-strength intervention that reduces the transmission rate of the wild type and a variant by the same amount (and therefore, keeping the relative strength constant), the relative speed of the variant is sensitive to the epidemic growth rate of the wild type strain---this challenges the commonly-made assumption that relative speed of new variants remains constant over time.
Differences in the generation-interval distributions further exaggerate this effect.
}

\DIFadd{Therefore, neglecting }\DIFaddend potential differences in the mean generation intervals between the variant and the wild type can bias estimates of the relative strength from incidence data\DIFdelbegin \DIFdel{;
...
\DIFaddbegin \DIFadd{Other studies have also tried to explain apparent changes in their estimates of relative strength using generation-interval-based arguments but sometimes gave inaccurate explanations. 
...
\DIFaddend We mostly considered idealized interventions of constant strength or constant speed.

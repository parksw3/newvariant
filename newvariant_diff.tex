...
The epidemic strength also determines the final size of an epidemic in a \DIFdelbegin \DIFdel{homogeneous population }\DIFdelend \DIFaddbegin \DIFadd{homogeneously mixing population under the mass-action assumption }\DIFaddend \citep{anderson1991infectious}.
...
\DIFdelbegin \DIFdel{Given a }\DIFdelend \DIFaddbegin \DIFadd{Before these interventions are introduced, the dynamics of pathogen spread can be characterized in terms of the }\DIFaddend pre-intervention kernel \DIFdelbegin \DIFdel{$K_{\mathrm{pre}}(\tau) = \RR g(\tau)$}\DIFdelend \DIFaddbegin \DIFadd{$K_{\mathrm{pre}}(\tau)$}\DIFaddend , which represents the rate at which an infected individual generates secondary infections $\tau$ time units after infection\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{.
...
Then, }\DIFaddend a constant-strength intervention reduces transmission by a constant factor $\theta$ \DIFdelbegin \DIFdel{across generation intervals }\DIFdelend \DIFaddbegin \DIFadd{throughout infection }\DIFaddend such that the post-intervention kernel is \DIFdelbegin \DIFdel{$K_{\mathrm{post}} = K_{\mathrm{pre}}(\tau)/\theta$}\DIFdelend \DIFaddbegin \DIFadd{$K_{\mathrm{post}}(\tau) = K_{\mathrm{pre}}(\tau)/\theta$}\DIFaddend .
In this case, the intervention strength $\theta$ must be greater than the epidemic strength $\RR$ to control the epidemic.
In contrast, a constant-speed intervention reduces transmission after infection by a constant rate $\phi$ \DIFdelbegin \DIFdel{across generation intervals: $K_{\mathrm{post}} = K_{\mathrm{pre}}(\tau) \exp(-\phi \tau)$}\DIFdelend \DIFaddbegin \DIFadd{throughout infection: $K_{\mathrm{post}}(\tau) = K_{\mathrm{pre}}(\tau) \exp(-\phi \tau)$}\DIFaddend .
In this case, the intervention speed $\phi$ must be greater than the \DIFaddbegin \DIFadd{pre-intervention }\DIFaddend epidemic speed $r$ to control the epidemic.
\DIFaddbegin \DIFadd{We note that the resulting post-intervention generation-interval distribution under a constant-speed intervention is }\emph{\DIFadd{not}} \DIFadd{equal to $g(\tau) \exp(-\phi \tau)$; instead $g(\tau) \exp(-\phi \tau)$ needs to be renormalized to integrate to 1 because the generation-interval distribution is a probability distribution.
}\DIFaddend 

\DIFaddbegin \DIFadd{These two idealized interventions, in turn, allow us to understand how holding $\RR$ or $r$ fixed can lead to different conclusions about epidemic control.
}\DIFaddend For example, if we assume epidemic strength $\RR$ is known, then variation in the generation-interval distribution does not change the estimated effectiveness of a constant-strength intervention.
...
\end{equation}
We can approximate this $r$--$\RR$ relationship by assuming that the generation-interval distribution is Gamma-distributed, and summarizing it using the mean generation interval $\bar{G}_x$ and the squared coefficient of variation $\kappa_x$ \DIFaddbegin \DIFadd{(equal to the reciprocal of the Gamma shape parameter) }\DIFaddend \citep{park2019practical}:
\begin{equation}
...
%DIF <  We thus ask: what factors affect the relative strength $\rho = \Rv/\Rw$ of a new variant, conditional on an observed relative speed $\delta$?
\DIFdelbegin \DIFdel{Inference of }\DIFdelend \DIFaddbegin \DIFadd{We thus ask: what factors affect }\DIFaddend the relative strength $\rho = \Rv/\Rw$ \DIFdelbegin \DIFdel{from the }\DIFdelend \DIFaddbegin \DIFadd{of a new variant, conditional on an observed }\DIFaddend relative speed $\delta$\DIFaddbegin \DIFadd{?
This inference }\DIFaddend depends on assumptions about the generation-interval distributions of both strains.
...
Unsurprisingly, we find that an increased speed of $\delta=0.1\pday$ for the variant is consistent with higher strength \DIFaddbegin \DIFadd{($\rho > 1$) }\DIFaddend than the wild type across \DIFdelbegin \DIFdel{the range of epidemiological conditions }\DIFdelend \DIFaddbegin \DIFadd{all five epidemiological scenarios }\DIFaddend considered (\fref{relstrength}).
However, the magnitude of relative strength $\rho$ is sensitive to assumptions about generation intervals:
For realistic values of $\kappa$ (excluding 0 and 1), the inferred relative strength $\rho$ ranges between 1.1 and 2.3 \DIFaddbegin \DIFadd{(across all five scenarios) }\DIFaddend when $\Gv$ is allowed to vary between $2/3$ and $3/2$ of \Gw.

In general, longer mean generation intervals of the new variant translate to higher values of $\rho$ (and vice versa\DIFaddbegin \DIFadd{;  \fref{relstrength}, bottom panels}\DIFaddend ), except when $\rv \leq 0$ (recall, we always assume $\rw<\rv$\DIFaddbegin \DIFadd{; \fref{relstrength}, top panels}\DIFaddend ).
...
When $\Rv=1$, the relative speed is insensitive to the generation-interval distribution of the variant because we always have $\rv=0$ \DIFaddbegin \DIFadd{(\fref{relspeed}, top-right panel)}\DIFaddend .
When $\Rv<1$, longer generation intervals of the variant lead to slower decay ($\rv$ closer to 0), and therefore, greater relative speed \DIFaddbegin \DIFadd{(\fref{relspeed}, top-left panel)}\DIFaddend .
...
For example, even when the generation-interval distributions are identical ($\Gv=\Gw$, in this case), changing $\Rw$ from 0.49 \DIFaddbegin \DIFadd{(\fref{relspeed}, top-left panel) }\DIFaddend to 1.27 (\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{\fref{relspeed}, bottom-right panel)---and }\DIFaddend thus $\Rv$ from 0.79 to 2.04\DIFdelbegin \DIFdel{) changes }\DIFdelend \DIFaddbegin \DIFadd{---changes }\DIFaddend the relative speed $\delta$ from 0.08 to 0.11 when $\kappa=0.2$ \DIFdelbegin \DIFdel{---and }\DIFdelend \DIFaddbegin \DIFadd{(and }\DIFaddend from 0.06 to 0.14 when $\kappa=0.5$\DIFaddbegin \DIFadd{)}\DIFaddend .
...
When the \DIFdelbegin \DIFdel{assumed }\DIFdelend \DIFaddbegin \DIFadd{wild type and the variant have the same (known) generation-interval distributions, the assumed generation-interval }\DIFaddend distribution matches the true distribution \DIFaddbegin \DIFadd{(\fref{Rtbias}D--F).
In this case}\DIFaddend , the estimated reproduction numbers match the true values (\fref{Rtbias}D); thus, their \DIFdelbegin \DIFdel{ratio }\DIFdelend \DIFaddbegin \DIFadd{estimated ratio $\hat{\rho}(t)$ }\DIFaddend remains constant and $\hat{\RR}_{\textrm{var}}(t)=\Rv(t)$ (\fref{Rtbias}E).
However, when the \DIFdelbegin \DIFdel{generation-interval distribution of the variant differs from the assumed distribution, the ratio changes over time (\fref{Rtbias}B,H).
If the }\DIFdelend true generation intervals of the variant have a shorter mean than the assumed distribution, we over-estimate $\Rv(t)$ during the growth phase and under-estimate $\Rv(t)$ during the decay phase (\DIFdelbegin \DIFdel{and conversely, \fref{Rtbias}A,G), which further translates to biases in the estimated relative strength }\DIFdelend \DIFaddbegin \DIFadd{\fref{Rtbias}A);
therefore, the estimated ratio $\hat{\rho}(t)$ changes over time }\DIFaddend (\fref{Rtbias}B\DIFdelbegin \DIFdel{,H}\DIFdelend \DIFaddbegin \DIFadd{).
Conversely, when the true generation intervals of the variant have a longer mean than the assumed distribution, we find similar biases in opposite directions (\fref{Rtbias}G--H}\DIFaddend ).
...
\DIFadd{While detailed contact tracing data can help narrow down uncertainties in the generation-interval distributions, there are additional complexities to comparing generation-interval estimates of different variants.
The generation intervals can be measured in two different ways: backward and forward \mbox{%DIFAUXCMD
...
While the forward distribution is more stable, changes in epidemic dynamics due to intervention measures and susceptible depletions can affect the shape of either distribution \mbox{%DIFAUXCMD
\citep{ali2020serial, nishiura2010time}}\hspace{0pt}%DIFAUXCMD
.
For example, comparing generation-interval estimates from different intervention periods will necessarily bias the true differences in the generation-interval distributions of different variants}\DIFaddend .

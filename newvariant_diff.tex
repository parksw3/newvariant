...
\DIFaddbegin \DIFadd{Before these interventions are introduced, the dynamics of pathogen spread can be characterized in terms of the pre-intervention kernel $K_{\mathrm{pre}}(\tau)$, which represents the rate at which an infected individual generates secondary infections $\tau$ time units after infection.
...
We note that the resulting post-intervention generation-interval distribution under a constant-speed intervention is }\emph{\DIFadd{not}} \DIFadd{equal to $g(\tau) \exp(-\phi \tau)$; instead $g(\tau) \exp(-\phi \tau)$ needs to be renormalized to integrate to 1 because the generation-interval distribution is a probability distribution.
}

\DIFaddend Given a pre-intervention kernel $K_{\mathrm{pre}}(\tau) = \RR g(\tau)$, which represents the rate at which an infected individual generates secondary infections $\tau$ time units after infection, a constant-strength intervention reduces transmission by a constant factor $\theta$ across generation intervals such that the post-intervention kernel is \DIFdelbegin \DIFdel{$K_{\mathrm{post}} = K_{\mathrm{pre}}(\tau)/\theta$}\DIFdelend \DIFaddbegin \DIFadd{$K_{\mathrm{post}}(\tau) = K_{\mathrm{pre}}(\tau)/\theta$}\DIFaddend .
In this case, the intervention strength $\theta$ must be greater than the epidemic strength $\RR$ to control the epidemic.
In contrast, a constant-speed intervention reduces transmission after infection by a constant rate $\phi$ across generation intervals: \DIFdelbegin \DIFdel{$K_{\mathrm{post}} = K_{\mathrm{pre}}(\tau) \exp(-\phi \tau)$}\DIFdelend \DIFaddbegin \DIFadd{$K_{\mathrm{post}}(\tau) = K_{\mathrm{pre}}(\tau) \exp(-\phi \tau)$}\DIFaddend .
In this case, the intervention speed $\phi$ must be greater than the epidemic speed $r$ to control the epidemic.

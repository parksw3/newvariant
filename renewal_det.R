...
renewal_det_speed <- function(R=2.5,
                              r1, r2,
                              hfun=function(t) 0,
                              theta=1.5,
                              N=40000,
  ...
  hvec <- sapply(tveclong, hfun)
  
  for (i in 2:length(tvec)) {
    j <- i + genmax
    
    Hvec <- exp(-rev(cumsum(rev(hvec[max(1, j-genmax):(j-1)])))*dt)
    
    Ivec1[j] <- Svec[i-1] * R * sum(Ivec1[max(1, j-genmax):(j-1)] * gen1[(genmax+1):2] * Hvec)/N
    Ivec2[j] <- Svec[i-1] * theta * R * sum(Ivec2[max(1, j-genmax):(j-1)] * gen2[(genmax+1):2] * Hvec)/N
    Svec[i] <- Svec[i-1] - Ivec1[i] - Ivec2[i]
    
    Rt1[i] <- Svec[i-1] * R * sum(gen1[(genmax+1):2] * Hvec)/N
    Rt2[i] <- Svec[i-1] * theta * R * sum(gen2[(genmax+1):2] * Hvec)/N
  }

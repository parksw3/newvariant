...
Other studies have considered the possibility that the faster growth rate of the new variant may be driven by \emph{shorter} generation intervals \citep{davies2021estimated}.
...
More information is clearly needed, however. And the story may be complicated: generation intervals depend on many other biological and behavioral factors, including for example self-isolation after symptom onset leading to shorter generation intervals.

Here, we explore how different assumptions about the relative generation time of the new variant affect estimates of its strength and speed.
...
Neglecting the (relatively slow) rate of new mutations, the current incidence of infection $i_x(t)$ caused by the wild type $w$ and the variant $v$) can be expressed in terms of their previous incidence $i_x(t-\tau)$ and the rate at which secondary cases are generated at time $t$ by individuals infected $\tau$ time units ago $K_x(t, \tau)$:
\begin{equation}

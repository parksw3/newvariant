...
Neglecting differences in generation-interval distributions can bias estimates of relative strength. \jd{Can we delete this? Is there a a good way to clarify?: \old\ which can vary systematically with changes in the incidence of infections caused by the variant.}
...
Analyses of new variants have characterized \emph{relative} strength (i.e., the ratio of reproduction numbers $\Rv/\Rw$, here called $\rho$) and speed (i.e., the difference between growth rates $\rv-\rw$, here called $\delta$) of the variants, where $\Rv$, $\Rw$ and $\rv$, $\rw$ represent reproduction numbers and growth rates at a given time.
Many analyses have particularly focused on changes in the \emph{proportion} of a new variant to estimate the relative speed \citep{switzerland2021variant, davies2021estimated, di2021impact, graham2021changes, leung2021early, volz2021transmission,zhao2021}\jd{Can we drop this?: \told{instead of taking the difference between their observed speed}--if we're looking at the same time period, these are probably equivalent, and otherwise it's not clear what we mean}.
...
\jd{I feel this explanation becomes more straightforward if we introduce $\hat K$ for the new kernel.}
...
On the other hand, a constant-speed intervention reduces transmission after infection by a constant rate $\phi$ across generation intervals: $K(\tau) \exp(-\phi \tau)$---in this case, the intervention speed $\phi$ must be greater than the epidemic speed $r$ to prevent onward transmission \citep{doi:10.1098/rspb.2020.1556}.
...
The recent emergence of the variant, and its breakthrough infections in previously immune individuals, adds further uncertainty to individual-level transmission dynamics and therefore the generation-interval of SARS-CoV-2 variants \citep{pulliam2021increased}.
...
\jd{We should probably change wild type to resident (res) by now.}
\jd{And then do we need to also say invader (inv)?. Bleah.}
\jd{Maybe just say resident and invader once or twice in the beginning, but tell the metaphorical story with $v$ and $w$ -- it's good notation, and easier.}
...
\jd{This is unbalanced; it can also be longer, because longer infections are generally assumed to seed proportionally more infections -- maybe leave it out here.}
...
We do note that we expect a distribution with higher coefficient of variation to allow for more early transmission, and thus to have qualitatively similar effects to a distribution with a shorter mean \citep{park2019practical} \jd{add: during periods of growth, when early events are more important than later events--or drop the sentence.}
...
\jd{Do we need the second equation?}
\begin{align}
...
We do not generally expect the relative speed $\delta$ to remain fixed if other factors governing epidemic spread are changing \jd{Drop? \old on the time scale of the invasion of a new variant relative to that of a wild type}.
...
For example, if the proportion of the population susceptible declines, or the average contact rate changes, while other factors remain unchanging, the relative strength $\rho$ is expected to remain fixed \citep{leung2017monitoring,leung2020empirical,di2021impact,leung2021early}; \jd{Drop? \old as we show in this section, these changes will imply a change in relative speed $\delta$}.
...
\jd{This is very deep, and I don't think there's any ``once again'' here -- meaning, we haven't explained this one yet. This is the effect of lower $\kappa$ for both being compared to the effect of longer GI for just one. In the non-comparative version, the effect of high $\kappa$ is on the influential end: it's like longer (shorter) GIs when disease is decreasing (increasing).}
...
\jd{Instead here doesn't quite work -- it's still about strength from speed because you're still observing $r(t)$ more directly and making assumptions about GI. \old
Instead of inferring relative strength from speed, one can directly estimate time-varying reproduction numbers $\RR(t)$ of the variant and the wild type from incidence data and directly compare their ratios when the incidence of infection and the generation-interval distributions are known}
...
Biases in slopes further translate to biases in intercepts; in theory, the regression line should go through the origin (because $\Rv = 0$ when $\Rw = 0$). \jd{Did you fit lines constrained through the origin or not? Need to be clear; what would a bias in intercept even mean?}
...
There were a (minor) discrepancy in the estimates of relative strength of the Alpha variant.
Mathematical analyses typically reported greater than a 1.4 fold increase in reproduction number for the Alpha variant ($\rho > 1.4$) whereas an independent analysis of secondary attack rates from contact tracing data suggested a 1.25--1.4 fold increase \citep{ukinvest}.
...
Other studies have also tried to explain apparent changes in their estimates of relative strength using generation-interval-based arguments. 
...
We have primarily focused on the differences in the mean generation interval but the differences in the amount of variability, characterized by the squared coefficient of variation $\kappa$, can also have important, but different implications \citep{blanquart2021selection}.
...
Other strength-like interventions, such as vaccination, can also have disproportionately smaller effects on new variants depending on their antigenic variability.
...
Although a few studies have tried to quantify both the generation- and serial-interval distributions of new SARS-CoV-2 variants, their conclusions on whether transmission time scale of new variants differs are conflicting \citep{pung2021serial, hart2021generation}---we note that these comparisons are inherently difficult especially due to temporal changes in generation- and serial-interval distributions due to dynamical and intervention effects \citep{ali2020serial,park2021forward}.

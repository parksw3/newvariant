...
We explore how differences in the generation-interval distributions affect the inference of relative strength $\rho$ and speed $\delta$ in light of recent evidence that the alpha variant may have a longer infectious period (and therefore, a longer generation interval) than prior dominant lineages.
...
However, if the variant has longer generation intervals than the wild type (\fref{strengthspeed}C--D), then the strength of the variant will be higher because we are holding the observed speed constant (\fref{strengthspeed}E).

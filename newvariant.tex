...
For example, the standard SEIR model can be equivalently expressed as a renewal equation with time-invariant $g(\tau)$ as shown in \citep{champredon2018equivalence}.
...
Previous studied have noted, in many contexts, that the realized generation intervals can contract due to susceptible depletion---a special case of strength-like changes---due to decreases in transmission even though the instantaneous generation-interval distribution remains unchanged.
In the context of SARS-CoV-2, assuming a time-invariant instantaneous generation-interval distribution is often appropriate given that the dynamics of its spread have been primarily driven by strength-like interventions, especially social distancing measures and vaccination.
...
\end{equation}
As our baseline scenario we assume $\rho = 1.61$, which is the value we obtain for $\delta=0.1\pday$ \citep{davies2021estimated}, $\rw=0\pday$, $\Gw = \Gv = 5\,\textrm{days}$, and $\kappa = 1/5$ \citep{ferretti2020quantifying}.

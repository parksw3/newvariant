...
Although the role of the generation interval (i.e., time between infection and transmission) in linking epidemic strength and speed has been previously well-established, many analyses of new SARS-CoV-2 variants overlook uncertainties in the generation-interval distributions as well as the possibility that their inferences of epidemic dynamics can depend on whether relative strength or speed are held constant.
Here, we explore how differences in the generation-interval distributions affect the inference of relative strength and speed in light of recent evidence that the B.1.1.7 variant may have a longer infectious period (and therefore, a longer generation interval) than prior dominant lineages.
...
We also assess which of the two assumptions (i.e., constant relative strength or speed) are more appropriate and further discuss implications for control.
...
Epidemic ``speed'' is characterized by the growth rate $r$, which has a unit of $1/\mathrm{time}$ and describes how fast a disease spreads at the population level.
...
Instead, epidemic strength can be inferred from the observed epidemic speed using the generation-interval distribution, an approach popularized by \citep{wallinga2007generation}.
The generation interval is defined as the time between infection and transmission and provides information about the individual-level transmission \citep{svensson2007note}.
...
For example, when epidemic strength is fixed, assuming longer generation intervals lead to a slower epidemic (lower $r$), making the epidemic look easier to control with speed-like interventions (e.g., contact tracing), which depend on hazard of isolation and therefore affects late transmission disproportionately.
When epidemic speed is fixed, assuming longer generation intervals lead to a stronger epidemic (higher $\RR$), making the epidemic look harder to control with strength-like interventions (e.g., social distancing), which reduce the hazard of infectious contact and therefore affecting transmission by an equal amount regardless of age of infection.
Therefore, it is critical to evaluate which assumption is more appropriate for inferring transmission advantages of new variants.

In addition, most studies have assumed that the previous dominant strain (wild type) and variants have identical generation-interval distributions, but recent evidence suggests that the B.1.1.7 variant may have a longer duration of infection: 13.3 days (90\% CI: 10.1--16.5) for the new variant and 8.2 days (90\% CI: 6.5--9.7) for the wild type \citep{kissler2021densely}.
...
\end{equation}
This framework provides a flexible way of modeling disease dynamics and generalizes compartmental model, such as the SEIR model \citep{heesterbeek1996concept, diekmann2000mathematical, roberts2004modelling, aldis2005integral,breda2012formulation, champredon2018equivalence}.
...
Under strength-like changes that reduce the hazard of transmission by a constant amount (e.g., susceptible depletion and decreased transmission rates), the instantaneous generation-interval distribution does not across change time \citep{fraser2007estimating}.
...
Previous studied have noted, in many contexts, that the realized generation intervals can contract due to susceptible depletion---a special case of strength-like changes---due to decreases in transmission \citep{kenah2008generation,nishiura2010time,champredon2015intrinsic}, even though the instantaneous generation-interval distribution remains unchanged.
While the instantaneous generation-interval distribution can change under speed-like changes that disproportionately affect late transmissions (see \cite{fraser2007estimating} and Section 6 for discussion), assuming a time-invariant instantaneous generation-interval distribution is often appropriate in the context of SARS-CoV-2, given that the dynamics of its spread have been primarily driven by strength-like interventions, such as social distancing measures \citep{flaxman2020Rt} and vaccination \citep{moore2021vaccination}.
Therefore, we neglect changes in the intrinsic generation-interval distribution over time for now and return to this idea in Section 6.
...
In the context of SARS-CoV-2, we are essentially assuming that the changes in susceptible pool, behavior, and contact rates are negligible over a short period of time.
...
\end{align}
where the logistic growth rate $\delta = \rv - \rw$ (i.e., the slope of the linear regression at the logit scale) corresponds to the relative speed of the epidemic.
When more than two strains are co-circulating, the growth rate at a logit scale is expected to change over time and does not correspond to the relative speed even when all strains exhibit exponential changes (see Supplementary Text);
...
Therefore, characterizing the spread of variants assuming constant relative speed (by fitting a standard logistic growth curve; e.g., \citep{campbell2021increased,switzerland2021variant}) should be used with care.
...
therefore, the instantaneous reproduction number only depends on transmission at time $t$ and is most appropriate for real-time evaluation of changes in trasmission, as argued by \citep{gostic2020practical}.
...
Here, we choose to focus here on the underlying dynamical mechanisms that may affect inference and assume that the incidence of infection is exactly known.
...
Finally, we synthesize our findings and illustrate the differences between constant-strength and constant-speed interventions using epidemic simulations using the renewal equation (\fref{control_sim}).
In particular, we consider a scenario in which a new variant is emerging ($\rv > 0\pday$ and $\Rw > 1$) while the incidence of infections caused by the wild type is constant ($\rw = 0\pday$ and $\Rw =1$) before interventions are reduced.
The constant-strength intervention then reduces both $\Rw$ and $\Rv$ by a constant factor $\theta$ beginning day 30, 
whereas the constnat-speed intervention isolates individuals infected with both the wild type and the variant at a constant hazard $\phi$ after day 30.
If the variant has a longer mean generation interval, the consant-strength intervention fails to suppress the spread of the variant (\fref{control_sim}A) because a stronger intervention ($>\phi$) to reduce a higher pre-intervention reproduction number below 1 (\fref{control_sim}B). 
The relative strength remains constnat under constant-strength intervention (\fref{control_sim}B), but the relative speed changes (\fref{control_sim}C) as discussed earlier in Section 4---in this case, the longer mean generation interval variant increases the relative speed when the intervention is introduced.
...
% SWP: in case you're wondering why this is commented out, I have moved this to section 5.

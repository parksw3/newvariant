...
Inferring relative strength (i.e., ratio between reproduction numbers, $\Rv/\Rw$) and relative speed (i.e., difference between growth rate, $\rv-\rw$) of new SARS-CoV-2 variants compared to their wild type is critical to predicting and controlling the course of the current pandemic.
Multiple studies have estimated the relative strength of new variants from their relative speed, but they typically neglect the possibility that the new variants have different generation intervals (i.e., time between infection and transmission), which determines the relationship between relative strength and speed.
However, the B.1.1.7 strain may have a longer infectious period than prior dominant lineages.
...
Finally, we discuss implications for control: if new variants have longer generation intervals then speed-like interventions (e.g., contact tracing) will be more relatively effective than strength-like interventions (e.g., social distancing), and therefore should be prioritized.

\section{Introduction}

Estimating variant epidemic strength and speed remains one of the key questions in controlling the spread of SARS-CoV-2 variants of concerns (VoCs) \citep{switzerland2021variant, davies2021estimated, di2021impact, leung2021early, volz2021transmission,zhao2021}.
Epidemic strength is characterized by the reproduction number $\RR$, which determines the number of new infections per average infection as well as the ``strength'' of intervention required to eliminate the disease \citep{anderson1991infectious}.
It also provides information about the final size of an epidemic under the homogeneous assumption \citep{anderson1991infectious}.
Epidemic speed is characterized by the growth rate $r$, which provides information about the population-level time scale of the epidemic.
Like epidemic strength, epidemic speed also determines conditions for disease elimination, where $r=0$ is a threshold like $\RR=1$ \citep{doi:10.1098/rspb.2020.1556}.
...
Analyses of new variants have prioritized characterizing \emph{relative} strength (ratios between reproduction numbers $\Rv/\Rw$) and speed (differences between growth rates $\rv-\rw$) of the variants.
...
In contrast, some studies have assumed a value of the relative strength of the novel variant and tried to predict its relative speed to determine if and when a new variant will become the dominant strain. \citep{davies2021estimated}.
...
For example, when we assume a fixed value of epidemic strength, longer generation intervals are associated with a slower epidemic (lower $r$), making the epidemic seem easier to control.
...
In practice, most studies have assumed that the generation-interval distributions do not differ between the existing strain and the variant, but recent evidence suggests that the B.1.1.7 variant may have a longer duration of infection: 13.3 days (90\% CI: 10.1--16.5) for the new variant and 8.2 days (90\% CI: 6.5--9.7) for the previously dominant variant, which we refer to as wild type \citep{kissler2021densely}.
...
However, linking strength and speed is complicated given that generation intervals depend on many other biological and behavioral factors, including for example self-isolation after symptom onset leading to shorter generation intervals.
We also do not know whether duration of infection of other variants will be same or different among each other.
\swp{JSW wanted to remove the last sentence but I feel like need a sentence that alludes to other variants.}

Here, we explore how different assumptions affect estimates of the relative affect estimates of its strength $\RR$ and speed $r$ for a new variant (e.g., B.1.1.7).
...
Assessing frequency dynamics has the advantage of being less sensitive to changes in testing and to other transient effects that might affect variants and wild type viruses similarly.
...
The relative strength of the new variant $\rho$ given speed of the wild type $\rw$, ratio between the mean generation interval of the new variant $\Gv$ and that of the wild type $\Gw$, and squared coefficient of variation in generation intervals $\kappa$.
Relative strength of the new variant $\rho$ is calculated using $\delta=0.1\pday$, $\Gw = 5\days$, and $\kappa = 1/5$.
...
To further explore how inference depends on underlying epidemiological conditions (i.e., whether the variant and the wild type are growing or decaying), we consider four epidemiological conditions: (1) $\rw < \rv < 0$, (2) $\rw < \rv = 0$, (3) $0 = \rw < \rv$, and (4) $0 < \rw < \rv$.
...
Instead of estimating relative strength from speed, it is possible to directly estimate ``time-varying'' or ``instantaneous'' reproduction numbers of the variant and the wild type from incidence data, and compare their ratios \citep{fraser2007estimating};
...
However, our analysis also shows how uncertainty in generation-interval distributions must be taken into account to obtain accurate estimates of the relative strength of variants.
If the variant has longer infectious period \citep{kissler2021densely}, and therefore extended generation-interval distributions, current estimates may be underestimating the true relative strength.
...
As more information about the transmission and immunity profiles of the new variant becomes available, then we advise fixing the relative strength and inferring the speed as it better matches biological mechanisms for its higher strength.

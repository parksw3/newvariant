...
Instead (or additionally), some studies have instead assumed a value of the relative strength and tried to predict its relative speed \citep{davies2021estimated,di2021impact}.
...
Here we focus on differences in mean generation intervals, assuming that both the squared coefficient of variation, and distribution shape of the variant are otherwise the same as for the wild type.
Changes in shape can be important and we do not investigate them here. We do not that, loosely, we expect a distribution with higher coefficient of variation to allow for more early transmission, and thus to have qualitatively similar effects to a distribution with a shorter mean in many cases.
...
\end{align}
where the relative speed of the variant $\delta = \rv - \rw$ can be estimated as the slope of the log odds of $p$  vs.~time.
When more than two strains are co-circulating, the picture is more complicated \citep{campbell2021increased}; we focus here on comparing strains two at a time. \jd{Simplified drastically here. Not sure what you added to supp or whether we need it.}
...
In all of these cases, we assume $\delta = \rv - \rw = 0.1\pday$.
...
\label{eq:rt}
\end{equation}
\jd{Isn't here where we should say pure strength or constant strength?:}
...
We consider constant-strength and -speed interventions calibrated to reduce $\Rv$ to 0.9 under the assumption that the variant generation intervals match those of the wild type(\fref{strengthspeed}A--B).
...
However, if the variant has longer generation intervals than the wild type (\fref{strengthspeed}C--D), then the strength of the variant will be higher (because we hold the observed speed  constant)(\fref{strengthspeed}E).
...
If the variant has a longer mean generation interval, the constant-strength intervention fails to suppress the spread of the variant (\fref{control_sim}A) because the longer generations imply a higher initial $\Rw$, and thus a stronger intervention is required to reduce $\Rw$ ber below 1 (\fref{control_sim}B). 

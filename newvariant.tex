...
Epidemic ``strength'' is measured by the reproduction number $\RR$ -- the average number of new infections caused by a ``typical'' infection \jd{Eliminate from here, stay sharp: as well as the strength of intervention required to eliminate the disease} \jd{Maybe add: disease can spread in a population only if $\RR>1$} \citep{anderson1991infectious} \jd{Cite Heesterbeek (for typical, also because they did the best job of characterizing classical Ro. Could drop anderson here, since it's already in the next s.}.
...
\jd{The middle of this s. is a bad place for this caveat. You should talk about it in terms of ``incidence of infection'', I think, and deal with it above here.}
...
\jd{If you add $\rw$ to the figure titles, you could skip this sentence and make life easier for readers.} Five epidemiological conditions assume $\rw=-0.15\pday$ ($\rw < \rv < 0$), $\rw=-0.1\pday$ ($\rw < \rv = 0$), $\rw=-0.05\pday$ ($\rw < 0 < \rv$), $\rw=0$ ($0 = \rw < \rv$), and $\rw=0.05\pday$ ($0 < \rw < \rv$).
}
...
To further explore how inference depends on underlying epidemiological conditions (i.e., whether the incidence of the variant and the wild type are growing or shrinking), we consider five epidemiological conditions: (1) $\rw < \rv < 0$, (2) $\rw < \rv = 0$, (3) $\rw < 0 < \rv$, (4) $0 = \rw < \rv$, and (5) $0 < \rw < \rv$. \jd{This looks like the right order to me; it does not seem to match F1.}
...
Even if we restrict the difference in generation interval to $<20\%$ and only consider scenarios with $\rv>0$ (bottom panels of \fref{relstrength}), we find that $\rho$ ranges between 1.3--1.8. \jd{Is it better to consider just one scenario here? $\rw=0$?}
...
\jd{Again, I'd like to see these ordered better. I also still like the idea of putting the value of Rwt in the figure titles (the geometric progression is cute, and we should keep those values, but maybe just mention it briefly and show the values to 2dp.} The relative speed of the new variant $\delta$ given strength of the wild type $\Rw$, ratio between the mean generation interval of the new variant $\Gv$ and that of the wild type $\Gw$, and squared coefficient of variation in generation intervals $\kappa$.
...
%% JD: How does this convoluted LaTeX not make you crazy?
...
\jd{Skip this:} Therefore, biases in the estimated relative strength will correlate with the strength of the variant, (i.e., whether $\Rv(t)$ is below or above 1).
...
\jd{intercept stuff is long and vague; is that because we're not confident of the pattern?}
...
In this case, we can control the spread of the variant when $\theta > \Rv$ or $\phi > \rv$, respectively. [[Cite speedstrength]]
...
\jd{Something is off here; we can't have both $r=0$.} Strength and speed are calculated assuming $\rw=0$, $\delta=0.1\pday$, $\rv=0$, $\Gy{wt}=5\,\textrm{days}$, and $\kappa=1/5$ for pre-intervention conditions.
Intervention strength and speed are chosen so that post intervention strength of the new variant is 0.9 when its mean generation interval is 5 days.9.
}
...
\end{equation}
we have $\ry{post} = \phi - \rv$. In particular, if speed of intervention is faster than the observed speed of spread (i.e., if $\phi > \rv$), we can control the epidemic regardless of the underlying generation-interval distribution.

\section{Discussion}

We explored how the generation-interval distribution shapes the link between relative strength and speed of an invadiang disease variant.
...
We can ask whether different estimates can be reconciled by generation intervals alone---for simplicity, we will assume that the variant and the wild type have identical generation-interval distributions as other studies have.
...
In this case, the former study's longer mean generation interval (which typically increases $\rho$) is counter-balanced by the effects of their wider distribution.
\jd{Not clear how to interpret this. Can we match both results under a gamma assumption changing only kappa and Gbar?}
In fact, we obtain $\rho=1.81$ from $\delta=0.104\pday$, $\rw=0$, $\Gv=\Gw=6.4\days$, and $\kappa=0.38$ under the gamma assumption, explaining the lack of differences in the estimates of $\rho$ despite the differences in the assumed generation-interval distributions.

However, the fixed generation generation interval assumed by \cite{davies2021estimated} is too narrow.
...
If we replace the assumed generation-interval distributions for \cite{volz2021transmission} with shorter generation interval estimates from Tianjin, China ($\Gw=2.57\days$ and $\kappa=1$; \cite{ganyani2020estimating}), we obtain $\rho=1.27$, which is consistent with the attack rate analysis \citep{ukinvest}.
...
\jd{I want to drop this here. Things are getting tangled, and this \P\ is about the equal-GI case.} Obtaining accurate estimates of $\rho$ will need to take into account changes in $\rho$ as well as underlying $\rw$.
...
First, models that assume a constant relative speed, such as standard logistic growth model, should be avoided for characterizing the growth of new variants---the relative speed changes over time depending on epidemiological conditions. \jd{need to reconcile this with advice above about Early.}
...
\jd{I changed this s., but maybe we can delete it; it's covered better in the next \P.} In addition, as we showed in our analysis, looking at absolute and relative values together can be critical to understanding dynamics. 

...
Inferring the relative strength (i.e., the ratio of reproduction numbers) and relative speed (i.e., the difference between growth rates) of new SARS-CoV-2 variants compared to their wild types is critical to predicting and controlling the course of the current pandemic.
We explore how differences in the generation-interval distributions affect the inference of relative strength and speed in light of recent evidence that the Alpha and Delta variants may have different infectiousness profiles (and therefore, the generation interval) from prior dominant lineages.
...
We do note that we expect a distribution with higher coefficient of variation to allow for more early transmission, and thus to have qualitatively similar effects to a distribution with a shorter mean in many cases \citep{park2019practical}.
...
(A--F) Pre-intervention (black) and post-intervention (colored) kernel of the new variant under constant-strength (A,C,E) and -speed (B,D,F) interventions when the variant has shorter (A--B), equal (C--D), or longer (E--F) mean generation interval than the wild type.

...
\begin{flushleft}{
	\Large
	\textbf\newline{
	  The importance of generation intervals in investigating dynamics and control of new SARS-CoV-2 variants }
}
\newline
\\
...
Inferring the relative strength (i.e., the ratio of reproduction numbers, $\rho=\Rv/\Rw$) and relative speed (i.e., the difference between growth rates, $\delta=\rv-\rw$) of new SARS-CoV-2 variants compared to their wild types is critical to predicting and controlling the course of the current pandemic.
We explore how differences in the generation-interval distributions affect the inference of relative strength $\rho$ and speed $\delta$ in light of recent evidence that the alpha \jd{suggesting that we use spelled-out greek names for variants, to reduce confusion with rendered greek letters for quantities} variant may have a longer infectious period (and therefore, a longer generation interval) than prior dominant lineages.
Neglecting these differences gives biased estimates of $\rho$, which can vary systematically with changes in the incidence of infections caused by the variant.
...
Estimating variant epidemic strength and speed remains a key question in understanding the threat of SARS-CoV-2 variants of concern (VoCs) \citep{switzerland2021variant, davies2021estimated, di2021impact,graham2021changes, leung2021early, volz2021transmission,zhao2021,campbell2021increased}.
...
Epidemic ``speed'' is characterized by the growth rate $r$, which has units of $1/\mathrm{time}$ and describes the exponential rate of disease spreads at the population level.
...
In particular, several studies have noted, in many contexts, that mechanisms that increase the mean generation interval can increase epidemic strength given observed epidemic speed \citep{eaton2014proportion,powers2014impact,weitz2015modeling,park2020time}.
...
\jd{``instead'' seems wrong here, since we cite davies2021estimated in both lists. What's going on?}
...
In this same case, however, the generation-interval distribution is expected to have negligible effects on the effectiveness of strength-like interventions (e.g., social distancing), which reduce the hazard of infectious contacts and therefore affect transmission by an equal amount regardless of age of infection.

Conversely, when epidemic speed $r$ is fixed, assuming longer generation intervals leads to a higher estimate of epidemic strength ($\RR$), making the epidemic look harder to control with strength-like interventions (but not with speed-like interventions, whose effectiveness is fixed if speed is fixed).

Most studies have assumed that the previous dominant strain (wild type) and variants have identical generation-interval distributions, but recent evidence suggests that the B.1.1.7 variant may have a longer duration of infection: 13.3 days (90\% CI: 10.1--16.5) for the new variant and 8.2 days (90\% CI: 6.5--9.7) for the wild type \citep{kissler2021densely}.
Longer duration of infection suggests that the mean generation interval of B.1.1.7 is likely to be longer than that of the wild type \citep{lloyd2001realistic,wearing2005appropriate,roberts2007model}.
...
However, linking strength and speed is complicated given that the generation-interval distribution depend on many factors including behavior: 
for example, self-isolation after symptom onset will lead to shorter generation intervals.

Here, we explore how different assumptions affect estimates of the relative strength ($\rho=\Rv/\Rw$) and relative speed ($\delta=\rv-\rw$) for a new variant, such as B.1.1.7 \jd{search and replace variant names if you agree}.
...
In practice, observed case reports are subject to reporting delays, which must be taken into account in order to correctly infer $r$ and $\RR$ \citep{goldstein2009reconstructing,gostic2020practical}.

Neglecting the (relatively slow) rate of new mutations and assuming a mass-action epidemic, the current incidence of infection $i_x(t)$ caused by each strain $x$---either the wild type (``wt'') or the variant (``var'')---can be expressed in terms of their previous incidence $i_x(t-\tau)$ and the rate at which secondary infections are generated at time $t$ by individuals infected $\tau$ time units ago $K_x(t, \tau)$:
\begin{equation}
...
\label{eq:instR}
\end{equation}
which is defined as secondary infections that would be caused by an individual infected at time $t$ if conditions were to remain the same \citep{fraser2007estimating}.
...
\jd{Where did -based come from? Do we like it?}
...
\end{align}
where the logistic growth rate $\delta = \rv - \rw$ (i.e., the slope of the linear regression of $p(t)$ at the logit scale) corresponds to the relative speed of the epidemic.
...
Since we are interested in the relationship between the true relative speed and strength, rather than the inference of relative speed from proportion data, our results are insensitive to the number of strains circulating in the population.
...
In all of these cases, we always assume $\rw < \rv$ to reflect the observed relative speed of $\delta = 0.1\pday$.
...
as we show in this section, this will imply a change in relative speed $\delta$.
...
\fref{relspeed} also shows that when $\rho$ is fixed relative speed depends on underlying epidemiological conditions---that is, whether the incidence of wild type and the variant are increasing or decreasing. 
For example, even when there are no differences in the generation-interval distributions ($\Gv=\Gw$, in this case), changing $\Rw$ from 0.49 to 1.27 (and thus $\Rv$ from 0.79 to 2.04) yields the relative speed $\delta$ between 0.08--0.11 for $\kappa=0.2$ and 0.06--0.14 for $\kappa=0.5$.
Therefore, characterizing the spread of variants assuming constant relative speed (e.g., by fitting a standard logistic growth curve \citep{switzerland2021variant}) should be used with care.
...
In practice, estimating the instantaneous reproduction number is complicated given that it requires incidence of infection.
...
Assuming that the instantaneous generation-interval distribution remains constant, the instantaneous reproduction numbers of the new variant and of the wild type can be estimated from their corresponding incidence curves---an approach popularized by \cite{cori2013new}:
\begin{equation}
\RR_x(t) = \frac{i_x(t)}{\int_0^\infty i_x(t-\tau) g_x(\tau) \dtau}.
\label{eq:rt}
\end{equation}
Under strength-like intervention measures that reduce transmission rates of both strains by a constant amount, we expect ratios between reproduction numbers to remain constant and correspond to the true relative strength: $\Rv(t)/\Rw(t) = \rho$ \citep{leung2017monitoring,leung2020empirical,di2021impact,leung2021early}.
...
(2) the wild type and the variant have the same generation-interval distributions (which match the known distribution, shown in \fref{Rtbias}D--F); and
...
(A--D) Pre-intervention (black) and post-intervention (colored) kernel of the new variant under constant-strength (A,C) and -speed (B,D) interventions when the variant has equal mean generation interval as the wild type (A--B) and when the variant has longer generation intervals than the wild type (C--D).
Vertical lines represent the mean generation interval.
(E) Pre-intervention $\Rv$ (black) and post-intervention epidemic strength $\Ry{post}$ (colored) conditional on the mean generation interval of the variant.
(F) Pre-intervention $\rv$ (black) and post-intervention epidemic speed $\ry{post}$ (colored) conditional on the mean generation interval of the variant.
...
(C,F) Proportion of incidence of infections caused by the variant ($\iv(t)/(\iw(t)+\iv(t))$) on a logit scale. 
...
As before, we consider a scenario in which a new variant is emerging ($\rv > 0\pday$ and $\Rw > 1$) while the incidence of infections caused by the wild type is constant ($\rw = 0\pday$ and $\Rw =1$) before interventions are reduced.
...
If the variant has a longer mean generation interval, the constant-strength intervention fails to suppress the spread of the variant (\fref{control_sim}A) because a stronger intervention ($>\phi$) to reduce a higher pre-intervention reproduction number below 1 (\fref{control_sim}B). 
...
In contrast, epidemic trajectories under the constant-speed intervention look identical regardless of the mean generation interval of the variant (\fref{control_sim}D). 
...
In this case, the relative speed of the variant also remains unaffected by the constant-speed intervention because the epidemic speed of the wild type $\rw$ and the variant $\rv$ are both reduced by equal amounts $\phi$, resulting in the post-intervention epidemic speed of $\rw-\phi$ and $\rv-\phi$, respectively. 
...
If new variants have a longer infectious period \citep{kissler2021densely}, and therefore extended generation-interval distributions, current estimates may underestimate the true relative strength.
...
As we demonstrated, if the variant and the wild type have different generation intervals, speed-like interventions such as contact tracing can affect them differently, causing the relative strength to change over time (but not necessarily the relative speed).

We considered two idealized interventions of constant strength and speed, but epidemiological impacts of real interventions are likely more complex.
...
However, analyses of SARS-CoV-2 dynamics have primarily relied on the constant-strength framework \citep{gostic2020practical,unwin2020state,brauner2021inferring}, even when modeling speed-like interventions such as self-isolation \citep{flaxman2020Rt,brett2020transmission}.
...
However, epidemiological conditions \textit{will} change over time in response to spreading new variants---in the context of SARS-CoV-2 infections, most of these responses will be strength-like, causing relative speed to change and invalidating this assumption.

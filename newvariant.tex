...
When the generation-interval distributions are asumed to be identical, we find that estimates of $\hat{\theta}$ decreases with $r_w$ whereas $\hat{\delta}$ increase with $\RR_w$.
...
This is not necesarily a bad news as the absolute speed ($r_v$) of the new variant still decreases as $\RR_v$ decreases.

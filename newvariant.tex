...
Epidemic speed, characterized by the growth rate $r$ provides information about the population-level time scale of the epidemic and, potentially provides information on when a new variant will replace the existing variant as the dominant strain.
...
\jd{I have started editing to make us a little less focused on B.1.1.7; we're not completely clear whether this is mostly a paper about B1, or whether it's just an example. I think we should focus less, and think also about new variants that might be coming. I stopped here, because this \P\ seems pretty clearly B1-focused and that part seems fine. Something to keep in mind.}
...
Other studies have considered the possibility that the faster growth rate of the new variant may be driven, in part, by shorter generation intervals \citep{davies2021estimated,volz2021transmission}.
...
\jd{It would be nice to have the R axes logged.}
...
In particular, if the true generation intervals have a shorter (or longer) mean than the assumed distribution, we over-estimate (or under-estimate) $\Rv(t)$ during the growth phase and under-estimate (or over-estimate) during the decay phase (\fref{Rtbias}D,G), which further translates to biases in the estimated relative strength (\fref{Rtbias}F,I). \jd{Not quite following the logic of how you're linking to the pictures. I note that panels C, E and H are not mentioned.}
...
When the assumed mean generation interval is longer (or shorter) than the that of the variant, we over-estimate (or under-estimate) the slope (\fref{Rtbias}H,L).
...
\jd{Maybe you didn't finish editing here? You've now mentioned F and three times, but still not E or H. And I can't find L despite repeatedly updating. It also doesn't quite seem to match the caption.}
...
As a baseline scenario, we assume $\rw=0$ and $\delta=0.1\pday$ (and therefore $\rv=0.1\pday$).
\jd{This sentence invites arguments, and we don't need it anyway:} In this case, the wild type is already under control, whereas the variant requires additional intervention measures to stop growing.

We consider two types of intervention:
an intervention of constant ``strength'', reduces transmission by a constant factor $\theta$ regardless of age of infection ($K_{\mathrm{post}}(\tau) = K_{\mathrm{pre}}(\tau)/\theta$); and an intervention of constant ``speed'' which reduces transmission after infection by a constant rate $\phi$ ($K_{\mathrm{post}}(\tau) = K_{\mathrm{pre}}(\tau) \exp(-\phi \tau)$).
...
\jd{This s.~feels repetitive, do we need it?:}

...
Here, we challenge a commonly-made assumption in estimating the transmission advantage of SARS-CoV-2 variants and argue that holding relative strength (rather than relative speed) constant is generally more appropriate.
In doing so, we use the generation-interval-based framework to compare how relative strength and speed change over time under two idealized interventions:
...
Differences in the generation-interval distributions can affect the effectiveness of these interventions as well as the realized transmission advantages of new variants.
...
Therefore, neglecting potential differences in the mean generation intervals between the variant and the wild type can bias estimates of the relative strength from incidence data.
...
We relied on parameter estimates for the Alpha variant throughout our analysis, but our qualitative conclusions can be applied to studying other SARS-CoV-2 variants, including the Delta variant.
...
Other studies have also tried to explain apparent changes in their estimates of relative strength using generation-interval-based arguemtns but sometimes gave inaccurate explanations. 

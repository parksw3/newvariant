...
In many contexts, previous studies have shown that inferences about disease dynamics (e.g., effectiveness of intervention strategy) can changes depending on whether it is assumed that $\RR$ or $r$ is known \citep{dushoff2020speed}.
...
Longer duration of infection is likely to provide opportunity to transmit for a longer period of time, meaning that the mean generation interval of the new variant is likely to be longer than that of the wild type.
More information is needed, however: generation intervals depend on many other biological and behavioral factors, including for example self-isolation after symptom onset leading .

Here, we explore how different assumptions about the new variant affect estimates of its strength and speed given the possibility that it might have longer generation intervals.
